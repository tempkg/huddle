...
# Seurat5
combined_rna <- MergeObjects(obj_list=seurat_list, col='Sample')
...
# Integration
integ_rna <- RunHarmonyIntegration(obj=combined_rna, norm_method='SCT', reduc='pca', new_reduc='harmony_rna', umap_reduc='rna.umap', max_dim=max_dim, res_clus=clus_res, seed=seed)
...
ggsave(paste0(outdir, '/scrna_sct2_integ.umap.sample.pdf'), width = 5.5, height = 4)

p <- DimPlot(integ_rna, group.by = 'seurat_clusters', reduction = "rna.umap", pt.size = 0.1, label=T)
ggsave(paste0(outdir, '/scrna_sct2_integ.umap.cluster.pdf'), width = 5.5, height = 4)


pdf(paste0(outdir, '/scrna_sct2_integ.elbowplot.pdf'), w=4, h=2)
ElbowPlot(integ_rna, ndims = max_dim, reduction = "harmony_rna")
dev.off()
