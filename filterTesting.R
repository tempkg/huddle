...
# v4
snrna_rpca <- Seurat4RNAIntegration(obj_list=snrna_list)
saveRDS(merged_snrna, file=paste0(outdir,'/snrna_sct_integ_rpca.withumap.v4.rds'))


# v5
snrna_integ <- RunNormalizeAndIntegrationRNA(obj_list=multiome_filtered_list, norm_method='SCT', integ_method='cca')
saveRDS(snrna_integ, file=paste0(outdir,'/snrna_sct_integ_cca.withumap.v5.rds'))

snrna_integ <- RunNormalizeAndIntegrationRNA(obj_list=multiome_filtered_list, norm_method='SCT', integ_method='rpca')
