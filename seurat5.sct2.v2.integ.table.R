...
for (name in dt$V1){
        ...
        # filter
        set_max_feature <- ifelse(seq == 'snRNA', 6000, 7500)
        cutoff <- CalMaxCutoffRNA(obj=seu, maxratioCount=0.99, maxratioFeature=0.9, set_max_feature=set_max_feature, revise_val=TRUE)
        print(cutoff)
        seu <- subset(x = seu,
