...
seu <- RunUMAP(seu, dims = 1:40, reduction.name='umap.rna', reduction.key='rnaUMAP_')
...
seu <- RunUMAP(seu, dims = 2:30, reduction='lsi', reduction.name='umap.atac', reduction.key='atacUMAP_')
...
p2 <- DimPlot(seu, reduction='umap.rna', pt.size=0.1)
p3 <- DimPlot(seu, reduction='umap.atac', pt.size=0.1)
print(p1)
print(p2)
print(p3)
dev.off()
