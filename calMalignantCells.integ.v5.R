...
for (s in unique(df_cnv_gene$sample)){
    ...
    # merge extra dataframe
    #ds_report <- merge(ds_report, d_cnv_ratio2, by='row.names', all.x=TRUE)


    # define malignant
    ds_report$cell_status <- 'Non-malignant'
    ds_report$cell_status[ds_report$cnv_ratio >= min_ratio | ds_report$cnv_gene_count >= min_cnv_gene] <- 'Malignant'
    #ds_report$cell_status[ds_report$cnv_ratio.max >= min_ratio & ds_report$cnv_gene_count.max >= min_cnv_gene & ds_report$cnv_ratio.by_allgenes >= min_ratio_by_allgene] <- 'Malignant'
...
print(dim(df_merged))
df_merged <- df_merged %>% select(sample, seurat_clusters, regions, n_gene_count, cnv_regions, cnv_gene_count, cnv_gene_count.max_in_region, cnv_ratio, cnv_ratio.max_in_region, cell_status)
