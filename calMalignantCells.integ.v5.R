...
min_ratio_by_allgene <- dyml$parameter$min_ratio_by_allgene
...
for (s in unique(df_cnv_gene$sample)){
    ...
    ds_report$gain_ratio.by_allgenes <- round(gain/total_genes * 100, 2)
    ds_report$loss_ratio.by_allgenes <- round(loss/total_genes * 100, 2)
    ds_report$cnv_ratio.by_allgenes <- round(total_cnv_genes/total_genes * 100, 2)
