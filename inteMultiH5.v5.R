...
# Integration
integ_rna <- RunNormalizeAndIntegrationRNA(obj_list=multiome_filtered_list, norm_method='SCT')
...
write.table(integ_atac@meta.data, file = paste0(outdir, '/integ_atac.metadata.xls'), sep = "\t", quote=F, col.names = NA)

p <- DimPlot(integ_atac, group.by = 'Sample', reduction = "umap.merged_atac", pt.size = 0.1)
...
pdf(paste0(outdir, '/integ_atac.umap.pdf'), width = 6.5, height = 5, useDingbats=FALSE)
print(p)
dev.off()
...
integrated_multiome <- CustomizedCombinationForIntegratedData(integ_rna=integ_rna, integ_atac=integ_atac, assay_use='SCT')
