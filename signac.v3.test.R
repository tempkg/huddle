...
signac_obj <- subset(signac_obj,
            subset = nCount_ATAC > ncount_atac_min &
                nucleosome_signal < nucleosome_signal &
                TSS.enrichment > tss_enrich_min &
                TSS.enrichment < tss_enrich_max &
                blacklist_ratio < blacklist_ratio &
                pct_reads_in_peaks > pct_reads_in_peaks  
            )

print(dim(signac_obj))
# Remove extreme outliers (top x%)
max_ncount <- quantile(signac_obj$nCount_ATAC, qt_cutoff)
print('[INFO] max ncount cutoff: ', max_ncount)

signac_obj <- subset(signac_obj, subset = nCount_ATAC < max_ncount)
print(dim(signac_obj))
