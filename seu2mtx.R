...
d_gene <- read.table(fgene, sep='\t', header=F, row.names=1)
used_genes <- intersect(data_genes, rownames(d_gene))
d_gene <- d_gene[used_genes,]
fwrite(d_gene, file=paste0(outdir, '/geneLoci.txt'), sep = "\t", quote=F, row.names=T, col.names=F)


# exp count for used_genes
assay_count <- seurat_obj@assays[[assay]]$counts[used_genes,cell_anno$cell]
fwrite(as.matrix(assay_count), file=paste0(outdir, '/expCount.txt'), sep = "\t", quote=F, row.names=T, col.names=NA)
