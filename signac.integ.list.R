...
for (name in sample_list){
        ...
        fseu <- paste0(sdir, '/', frds)
        temp_seu <- readRDS(fseu)

        # make data list
        snatac_macs2_list <- append(snatac_macs2_list, temp_seu)
...
if (method == 'harmony'){
    integ_atac <- ATACIntegrationHarmony (snatac_list=snatac_peak_list, new_reduc='harmony_atac', max_dim=max_dim, res_clus=0.4, seed=seed)
} else {
    integ_atac <- ATACIntegration(snatac_list=snatac_peak_list, new_reduc='integrated_lsi', max_dim=max_dim, res_clus=0.4, seed=seed)
}
