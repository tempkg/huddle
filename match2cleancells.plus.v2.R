...
PieChart_LineageBarcodeAndCellCounts <- function(data='')
{
    ...
    mycolors["Mixed"] <- "#d8d8d8"
    mycolors["Lineage1"] <- "#e21f26"

    p <- ggplot(data = data, aes(x = '', y = ncell, fill = lineage)) +
...
p <- PieChart_LineageBarcodeAndCellCounts(d_freq)
ggsave(paste0(outdir, '/piechart.lineage_cellCount.pdf'), w=0.9, h=0.9)
...
# barplot
Barplot_LineageBarcodeAndCellCounts <- function(data='')
{
...
p <- Barplot_LineageBarcodeAndCellCounts(d_freq)
ggsave(paste0(outdir, '/barplot.lineage_cellCount.pdf'), w=2, h=1.6)
