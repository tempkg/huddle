...
# filter - multiplet (optional)
if (nchar(rm_cells) > 0){
    d_poorcells <- read.table(rm_cells, sep='\t', header=F)
    colnames(d_poorcell) <- c('cell', 'sample')
    multiome_filtered_list <- base::lapply(X = multiome_filtered_list, FUN = function(x) { x <- FilterCellsByTable(obj=x, df=d_poorcells) })
}
...
integrated_multiome <- MultiomeIntegration_v1(obj_list=multiome_filtered_list, snatac_obj=integ_atac)
...
saveRDS(integrated_multiome, file=paste0(outdir,'/multiome_integrated_plus.seurat5.rds'))
