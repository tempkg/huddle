...
for (g in unique(info$group)){
    ...
    sub_multiome_wnn <- RunWNNPlus(obj=subseu, reduc_rna=reduc_rna, reduc_atac='harmony_atac', dim_max_rna=dim_rna, dim_max_atac=dim_atac, weight_name='SCT.weight', res_clus=0.4, seed=seed)

    saveRDS(multiome_wnn, file=paste0(outdir,'/', g, '.multiome_integrate.', dim_rna, '_', dim_atac, '.rds'))

    p <- DimPlot(sub_multiome_wnn, reduction = 'wnn.umap', group.by = 'Sample', pt.size = 0.1)
